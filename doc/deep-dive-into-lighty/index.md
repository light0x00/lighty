...
```txt
                            EventHandler                                    inbound pipeline(A set of ChannelHandlers
...
- `InboundChannelHandler`, 用于处理输入, 构成入方向 pipeline. 
- `OutboundChannelHandler`, 用于处理输出, 构成出方向 pipeline.
- `DuplexChannelHandler`, 同时处理输入/输出, 分别构成出入方向 pipeline.
