 ...
 */
public interface ChannelInboundHandler extends ChannelObserver {

    static Invocation buildInvocationChain(ChannelContext context, List<ChannelInboundHandler> pipelines, Consumer<Object> receiver) {
        Invocation invocation = receiver::accept;

        for (int i = pipelines.size() - 1; i >= 0; i--) {
            ChannelInboundHandler pipeline = pipelines.get(i);
