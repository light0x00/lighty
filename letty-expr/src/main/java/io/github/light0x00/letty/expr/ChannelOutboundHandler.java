 ...
 */
public interface ChannelOutboundHandler extends ChannelObserver {

    static OutboundInvocation buildInvocationChain(ChannelContext context, List<ChannelOutboundHandler> pipelines, BiConsumer<Object, ListenableFutureTask<Void>> receiver) {
        OutboundInvocation invocation = receiver::accept;

        for (int i = pipelines.size() - 1; i >= 0; i--) {
            ChannelOutboundHandler pipeline = pipelines.get(i);
